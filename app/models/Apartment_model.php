...
class Apartment_model extends Model {
    ...
    public function checkname( $name ) { 
        return $this->db->table('room')->where('name',$name)->not_where('category', 'RESORT')->get();
    }
    ...
    public function check_availability() {
        return $this->db->table('booked_dates as bdt')->select('date, bdt.room_id as room_id')->select_sum('bdt.qty', 'qty')->inner_join('room as r','bdt.room_id = r.id')->where('category', 'APARTMENT')->group_by(['room_id', 'date'])->get_all();
    }

    public function payment() {
        return $this->db->table('payment as pay')->select('pay.id as id, pay.photo as photo, pay.reference as reference, downpayment')->inner_join('reservations as res', 'pay.reference = res.reference')->inner_join('room as r', 'res.room_id = r.id')->where('category', 'APARTMENT')->get_all();
    }
